...
public class ProductionLineController extends BaseController {
     ...
     * 跳转打印产线二维码页面
     */
    @GetMapping("/printLineCode")
    public String printLineCode() {
        return prefix + "/lineCode";
    }
     ...
     */
    @RequiresPermissions("production:productionLine:add")
    @PostMapping("/changeOrderTag")
    @ResponseBody
    public AjaxResult changeOrderTag(ProductionLine line) {
        try {
            return toAjax(productionLineService.changeOrderTag(line));
        } catch (BusinessException e) {
            return error(e.getMessage());
        }
    }
