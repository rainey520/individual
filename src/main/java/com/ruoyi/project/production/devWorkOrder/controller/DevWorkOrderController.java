...
public class DevWorkOrderController extends BaseController {
    ...
    public String workMainMesConfig(ModelMap map) {
        map.put("cType", ImportConstants.TYPE_WORK_MES);
        ImportConfig importConfig = configService.selectImportConfigByType(ImportConstants.TYPE_WORK_MES);
     ...
     * 查看工单生产的产品文件信息
     */
    @GetMapping("/mesFileInfo")
    public String mesFileInfo(String productCode, ModelMap map) {
        int id = 0;
        DevProductList product = productService.selectProductByCode(productCode);
        if (product != null) {
            id = product.getId();
        }
        map.put("saveId", id);
        map.put("saveType", FileConstants.FILE_SAVETYPE_PRO_MES);
        return "product/list/pnMesFile";
    }
}
